...
with open('pidOut.csv','r+') as f:
	for line in f:
		...
		z.append(float(line.split('\t')[14])*5)
		...
		gas_force.append(float(line.split('\t')[4])/500)
		gas_i_store.append(float(line.split('\t')[32]))

plt.figure(1)                # the first figure

#New trial
plt.subplot(2,1,1)
plt.plot(tmp_t, gas_force,'y', tmp_t, z, 'k', tmp_t, gas_i_store, 'r', tmp_t, vel_curr, 'm', tmp_t, vel_target, 'b')
plt.title('Gas force and gas_i_store')
plt.xlabel('time')
plt.axis([0, 90, 0, 40])
